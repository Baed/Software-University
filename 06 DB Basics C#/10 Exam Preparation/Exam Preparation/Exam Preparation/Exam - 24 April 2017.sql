...
USE WMS;
*/
--SELECT * FROM Mechanics, Clients, Models, Jobs, Orders, Vendors, Parts, OrderParts, PartsNeeded
...
SELECT CONCAT(c.FirstName, ' ', c.LastName) AS [Client], 
             DATEDIFF(DAY, IssueDate, '24 April 2017') AS [Days going], j.Status
FROM Clients AS c
...
FROM
(
	SELECT p.Price, op.Quantity, DATEDIFF(WEEK, o.IssueDate, '24 April 2017') AS [Weeks]
	FROM Parts AS p
...
FROM Parts AS p
	 ...
	 LEFT JOIN Orders AS o ON o.JobId = j.JobId
	 LEFT JOIN OrderParts AS op ON op.PartId = o.OrderId
WHERE j.Status <> 'Finished'
...
BEGIN
	...
	BEGIN
		...
		WITH cte_OrderDataFromOrderParts(OrderId, PartId, Quantity)
			 AS (SELECT OrderId, PartId, SUM(Quantity) AS Quantity
				 FROM OrderParts
				 WHERE OrderId =
							   (
									SELECT OrderId
...
WITH cte_JoinedGroupedTables
	 AS (SELECT m.MechanicId, v.VendorId, SUM(op.Quantity) AS [PartsForMechanicByVendor]
		 FROM Mechanics AS m
			  JOIN Jobs AS j ON j.MechanicId = m.MechanicId
	 ...
	 SELECT CONCAT(m.FirstName, ' ', m.LastName) AS [Mechanic],
	        v.Name AS [Vendor], cte.PartsForMechanicByVendor AS [Parts], 
